...
static int push_job(int fd, T4Shared *shared, const char *path, int depth) {
    ...
    if (shared->job_count < T4_MAX_JOBS && !shared->done) {
...
static int process_directory(int fd, T4Shared *shared, int worker_id, const T4Job *job) {
    DIR *dir;
    struct dirent *entry;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode)) {
            if (shared->max_depth < 0 || job->depth + 1 <= shared->max_depth) {
                push_job(fd, shared, child_path, job->depth + 1);
            }
        } else if (S_ISREG(st.st_mode)) {
            emit_file_record(fd, shared, worker_id, child_path, &st);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (pop_result == 1) {
            ...
            process_directory(fd, shared, cfg.worker_id, &job);
